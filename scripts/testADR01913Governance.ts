...
export async function runGovernanceTests() {
  ...
  try {
    ...
    assert(!filteredBatch.some((item) => item.status === "PENDING_REVIEW" || item.status === "REJECTED_FINDING" || item.status === "GENERATED"), "Filtro de Ingesta: PENDING_REVIEW, REJECTED_FINDING y GENERATED filtrados strictly");
