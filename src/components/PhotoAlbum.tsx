...
export function PhotoAlbum({
  ...
  const handleCompleteMultipleStreetViewCaptures = useCallback(async (payloads: StreetViewCapturePayload[]) => {
    setIsSvPickerOpen(false);
    try {
      let savedCount = 0;
    ...
    try {
      ...
      const photosToExportData = photosToExport.map((p) => {
        ...
        return {
          ...
          humanValidationStatus: p.humanValidationStatus,
          validationSource: p.validationSource,
          lineage: p.lineage,
          ...
          acquisitionMode: (p as any).acquisitionMode,
          epistemic: (p as any).epistemic,
          epistemicIntegrity: (p as any).epistemicIntegrity,
          isSimulated: (p as any).isSimulated,
          isConnectivityOnly: (p as any).isConnectivityOnly,
          status: (p as any).status,
          estado: (p as any).estado,
          estado_revision: (p as any).estado_revision,
          analystValidationStatus: (p as any).analystValidationStatus,
      ...
      {svFlowTarget && (
        <>
          ...
          <StreetViewPanoramaPicker
            key={svFlowTarget ? `${svFlowTarget.lat}-${svFlowTarget.lng}` : "idle"}
            isOpen={isSvPickerOpen}
            lat={svFlowTarget ? svFlowTarget.lat : 0}
            lng={svFlowTarget ? svFlowTarget.lng : 0}
            onClose={() => {
              setIsSvPickerOpen(false);
              setSvFlowTarget(null);
            }}
